...
Abbiamo inoltre ricercato l'andamento di V$_{out}$(V$_{in}$) eseguendo le misure mostrate in \tab{risposta_ampiezza}. Un grafico è mostrato in \fig{risposta_ampiezza}. Un fit lineare ($V_{out}= mV_{in} + q$) presenta come risultati un'amplificazione $m = 10680 \pm 140$, non in accordo con il valore atteso (noto a partire dalla funzione di fit, posta alla frequenza di lavoro $f_L = 3.23 \pm 0.08$ kHz) $m_{att} = 9000 \pm 600$; un'intercetta $q = 10 \pm 9$ mV, $cov = -0.67$; $\chi^2 /dof = 0.2/3$.
\begin{table}[h]
	\centering
	\begin{tabular}{cccc}		
