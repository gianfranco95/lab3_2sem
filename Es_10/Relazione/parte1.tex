...
\begin{table}[h]
	\centering
	\begin{tabular}{*{5}{S[table-figures-exponent = 2]} }
		{} & {Valore misurato} & {Incertezza} & {condizioni di lavoro raccomandate (dal datasheet)}&{} \\
		\midrule
    $V_{IH}$ [V] 	  & 1.5 & 0.1  & 2 & min\\
    $V_{IL}$ [V]	  & 1.0  & 0.1  & 0.8 & max \\
    $V_{OH}$ [V]	  & 4.1  & 0.1  & 2.7 ($\rhd$3.4)  & min(typ)\\ %
    $V_{OL}$ [mV]     & 114  & 4  & 400 & max(Iol=4mA)\\
    $I_{IH}$ [$\mu$A] & 3  &  1  & 20  & (max)\\
    $I_{IL}$ [mA]     & -0.25 & 0.03  & -0.4 & (max)\\
    $I_{OH}$ [$\mu$A] & -37 & 1  & -400  & (max)\\
    $I_{OL}$ [$\mu$A] & 147 & 4  & 8000 & (max)\\
