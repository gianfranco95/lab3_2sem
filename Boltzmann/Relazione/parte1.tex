...
\begin{table}[h]
	...
	\label{t:pre-amp}
\end{table}
%In \fig{pre-amp} è mostrato il plot di Bode per l'amplificazione. Abbiamo realizzato un fit lineare, numerico, a due parametri limitatamente alla parte di sinistra, dove l'amplificazione risulta circa costante. Abbiamo ottenuto $m = ?? \pm ??$ dB/decade, compatibile con 0; $q = ?? \pm ??$ dB/decade, pari all'amplificazione per le frequenze passanti; $cov = ??$; $\Chi^2/dof = ??/??$. Un fit analogo è stato svolto per la parte lineare a destra, ottenendo $m = ?? \pm ??$ dB/decade, compatibile con il valore atteso; $q = ?? \pm ??$ dB/decade; $cov = ??$, $\Chi^2/dof = ??/??$.\\ %valori fit
...
Dall'intersezione delle due rette e propagando opportunamente gli errori è possibile ottenere il valore della frequenza di taglio $f_T = ?? \pm ??$ kHz, in accordo con il valore atteso $f_0 = 6.4$ kHz.
...
In \fig{passabanda} è mostrato il grafico dell'amplificazione in funzione della frequenza (plot di Bode). Abbiamo realizzato un fit con la funzione di trasferimento attesa ???, ottenendo ...\\ % risultati fit, amplificazione, larghezza di banda
